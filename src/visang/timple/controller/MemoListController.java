...
public class MemoListController {
	...
	@Resource(name="MemoListSV") // 서비스 이름 가져오기
	private IMemoList dao;
	
	// 게시판 리스트 불러오기
	@RequestMapping(value="/MemoList.vs", method={RequestMethod.GET})
	public String list(HttpServletRequest req, HttpSession session) {
		...
		List<MemoListDTO> memo = dao.getMemoListPerOnePage(map); // 한페이지에 출력될 게시물들
		...
		// a. 총 게시물 수 - total
		// b. 총 페이지 수 - pages
		HashMap<String, Integer> mList = dao.pageBar(memberId); 
		
		// System.out.println("총 게시물 수" + bdto.getTotal() + " 총 페이지 수 : " + bdto.getPages());
		
		int blockSize = 10; // 페이지 바의 페이지 갯수
		System.out.println("test : " + mList.keySet().toString());
		totalCount = mList.get("total");
		totalPage = mList.get("pages");
